...
# xact 2
payment_warehouse_select = session.prepare('SELECT * FROM warehouses WHERE w_id=?')
payment_warehouse_update = session.prepare('UPDATE warehouses SET w_ytd=? WHERE w_id=?')
payment_district_select = session.prepare('SELECT * FROM districts WHERE d_w_id=? AND d_id=?')
payment_district_update = session.prepare('UPDATE districts SET d_ytd=? WHERE d_w_id=? AND d_id=?')
payment_customer_by_wd_select = session.prepare('SELECT * FROM customer_by_wd WHERE c_w_id=? AND c_d_id=? AND c_id=?')
payment_customer_by_wd_update = session.prepare('UPDATE customer_by_wd SET c_balance=?, c_ytd_payment=?, c_payment_cnt=? WHERE c_w_id=? AND c_d_id=? AND c_id=?')

# xact 3
delivery_order_by_wd_select_min = session.prepare('SELECT MIN(o_id) FROM order_by_wd WHERE o_w_id=? AND o_d_id=? AND o_carrier_id = -1') # NULL is replaced by -1
...
get_next_available_order_number_prepared = session.prepare(
    'SELECT D_NEXT_O_ID FROM districts WHERE D_W_ID=? AND D_ID=?')
...
get_w_name = session.prepare('SELECT W_NAME FROM warehouses WHERE W_ID=?')
